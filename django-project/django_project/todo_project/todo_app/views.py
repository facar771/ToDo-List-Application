...
from .forms import ListForm , ListUser
...
def login(request):
    if request.method == "POST":
        form1 = ListUser(request.POST or None)
        if form1.is_valid:
            form1.save()
            todo_user = User.objects.all()
            return render(request,"todo_app/login.html",{'todo_user':todo_user})
    else:
        todo_user = User.objects.all()
        return render(request,"todo_app/login.html",{'todo_user':todo_user})
